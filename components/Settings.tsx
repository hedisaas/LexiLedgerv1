...
import { BusinessProfile, GlossaryTerm, TranslationJob, Expense } from '../types';
import { Save, Upload, Building2, User, MapPin, FileText, Phone, Mail, CreditCard, BookOpen, Plus, Trash2, Download, FileSpreadsheet } from 'lucide-react';
import { Lang, translations } from '../locales';
import { getGlossary, addGlossaryTerm, deleteGlossaryTerm } from '../services/tmService';
...
const Settings: React.FC<SettingsProps> = ({ profile, onSave, jobs, expenses, lang, userRole }) => {
  ...
  const handleAddTerm = (e: React.FormEvent) => {
    ...
    const term: GlossaryTerm = {
      ...
      langPair: 'en-fr' // default for now
    };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 bg-slate-50">
          <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2"><BookOpen className="w-5 h-5 text-indigo-600"/> {t.manageGlossary}</h2>
          <p className="text-sm text-slate-500 mt-1">Define terms for consistent translations across documents.</p>
        </div>
        
        <div className="p-6">
          <form onSubmit={handleAddTerm} className="flex flex-col md:flex-row gap-4 mb-6 items-end bg-slate-50 p-4 rounded-lg border border-slate-100">
             <div className="flex-1">
               <label className="block text-xs font-bold text-slate-500 uppercase mb-1">{t.sourceTerm}</label>
               <input type="text" placeholder="e.g. Lawyer" className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500 outline-none" value={newTerm.source} onChange={e => setNewTerm({...newTerm, source: e.target.value})} />
             </div>
             <div className="flex-1">
               <label className="block text-xs font-bold text-slate-500 uppercase mb-1">{t.targetTerm}</label>
               <input type="text" placeholder="e.g. Avocat" className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500 outline-none" value={newTerm.target} onChange={e => setNewTerm({...newTerm, target: e.target.value})} />
             </div>
             <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 h-[38px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {glossary.map(term => (
                  <tr key={term.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3 font-medium text-slate-900">{term.source}</td>
                    <td className="px-4 py-3 text-slate-700">{term.target}</td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => handleDeleteTerm(term.id)} className="text-slate-300 hover:text-rose-500 transition-colors"><Trash2 className="w-4 h-4" /></button>
                    </td>
                  </tr>
                ))}
                {glossary.length === 0 && (
                  <tr><td colSpan={3} className="px-4 py-8 text-center text-slate-400">No terms in glossary yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
